...
namespace Kwetter.Services.Common.Infrastructure.Messaging
{
    ...
    internal class QueueReaderService : IHostedService, IDisposable
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            {
                if (HandleMessage(evt2, cancellationToken))
                {
        ...
        private bool HandleMessage(BasicDeliverEventArgs message, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    handler.HandleMessageAsync(messageType, message.Body.ToArray(), cancellationToken).GetAwaiter().GetResult();
                }
