...
<template>
  <div class="container flex justify-center h-screen px-4 py-8 mx-auto text-center lg:px-0 dark">
    <div class="w-full sm:w-80">
      ...
      <h1 class="pt-10 text-4xl font-bold text-center dark:text-gray-100">Register to Kwetter</h1>
      <form @submit.prevent="register">
        <input
          v-model="input.email"
          type="text"
          placeholder="Email"
          class="w-full px-2 py-4 my-6 text-xl transition-colors duration-75 border-2 rounded border-lighter dark:border-dark focus:outline-none dark:bg-black dark:text-gray-100 focus:border-red dark:focus:border-red"
        />
        <input
          v-model="input.username"
          type="text"
          placeholder="Username"
          class="w-full px-2 py-4 mb-6 text-xl transition-colors duration-75 border-2 rounded border-lighter dark:border-dark focus:outline-none dark:bg-black dark:text-gray-100 focus:border-red dark:focus:border-red"
        />
        <input
        ...
        <button
          ...
          :disabled="inputEmpty"
        >
          <span class="text-lg font-semibold text-gray-100">Register</span>
        </button>
        <span v-if="loading">Loading...</span>
        <router-link to="/login">
          <span class="text-red">Sign in for Kwetter</span>
        </router-link>
      </form>
    </div>
  </div>
